...
{
  $defaults_cli = { 'path' => "${php::params::config_root}/cli/php.ini", require => Class['php::packages'] }
  $result_cli = deep_merge($php::params::default_config, $settings)
  create_ini_settings($result_cli, $defaults_cli)

  if($php::fpm) {
    $defaults_fpm = { 'path' => "${php::params::config_root}/fpm/php.ini", require => Class['php::fpm'] }
    $result_fpm = deep_merge($php::params::default_config, $settings)
    create_ini_settings($result_fpm, $defaults_fpm)
  }

  $defaults_opc = { 'path' => "${php::params::config_root}/mods-available/opcache.ini", require => Class['php::packages']}
